...
# 模型预估目标
PXTRS = ["expand_xtr", "like_xtr", "reply_xtr", "continuous_expand_xtr"]
# 最终krp部署时会覆盖这个kess_name
infer_kess_name = "grpc_commentZoneContinuousExpandInfer"
# 请求带过来的attrs
item_attrs_from_req = [
...
model_btq_prefix = "total_comment_model"
embed_server_shards = 1
# krp 上 embedding_server的kess_name
embed_kess_name = "grpc_commentZoneContinuousExpandEmb"
...
class PredictServerFlow(LeafFlow, KuibaApiMixin, MioApiMixin, OfflineApiMixin, GsuApiMixin, UniPredictApiMixin, KgnnApiMixin):
  ...
  def predict_with_mio_model(self, **kwargs):
    ...
    extra_inputs = [
    #   ("mmu_comment_content_emb", "mmu_comment_content_emb", False, 256),
    ...
    return self.extract_kuiba_parameter(
      ...
      .enrich_attr_by_lua(
        ...
        export_item_attr=["fre_grouped_predict_reply_score", "fre_grouped_quality_v2_score",
...
# Infer服务最终返回的值
service.IGNORE_UNUSED_ATTR=["fre_grouped_predict_reply_score", "fre_grouped_predict_like_score"]
service.return_item_attrs(["expand_xtr", "like_xtr", "reply_xtr", "profile_xtr", "continuous_expand_xtr"])
