...
field_input = tf.concat([user_embedding, comment_id_embedding], -1)
field_input_norm = tf.nn.l2_normalize(field_input, axis=-1)
expand_xtr = simple_dense_network("expand_xtr", field_input_norm, [256, 128, 64, 1], 0.0, act=tf.nn.leaky_relu, last_act=tf.nn.sigmoid)
...
if args.mode == 'train':
    ...
    # hot_weight = tf.where(recall_type > 0, sample_weight, zeros)
    # temporal_weight = tf.where(recall_type > 0, zeros, sample_weight)
    eval_targets = [
        ('expand_predict', expand_xtr, expand_label, ones, "auc"),
        ('like_predict', like_xtr, like_label, ones, "auc"),
        ('reply_predict', reply_xtr, reply_label, ones, "auc"),
        # ('hot_expand_predict', expand_xtr, expand_label, hot_weight, "auc"),
        # ('hot_like_predict', like_xtr, like_label, hot_weight, "auc"),
        # ('hot_reply_predict', reply_xtr, reply_label, hot_weight, "auc"),
        # ('temporal_expand_predict', expand_xtr, expand_label, temporal_weight, "auc"),
        # ('temporal_like_predict', like_xtr, like_label, temporal_weight, "auc"),
        # ('temporal_reply_predict', reply_xtr, reply_label, temporal_weight, "auc")
    ]
