...
if args.mode == 'train':
    ...
    comment_id_embedding = kai.nn.new_embedding("c_id_embedding", dim=64, slots=[202])
    ...
    comment_content_embedding = tf.reduce_sum(tf.reshape(kai.nn.new_embedding("comment_content_embedding", dim=8, expand=16, slots=[212]), [-1, 16, 8]), axis=1)
else:
    ...
    comment_id_embedding = config.new_embedding("c_id_embedding", dim=64, slots=[202])
    ...
    comment_content_embedding = tf.reduce_sum(tf.reshape(config.new_embedding("comment_content_embedding", dim=8, expand=16, slots=[212]), [-1, 16, 8]), axis=1)
...
field_input = tf.concat([user_embedding, comment_id_embedding, comment_label_embedding, position_embedding, comment_tag_embedding, comment_content_embedding], -1)
