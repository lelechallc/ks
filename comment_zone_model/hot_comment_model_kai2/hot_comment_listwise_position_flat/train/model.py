...
if args.mode == 'train':
    ...
    # 下面这些输入的维度加和
    feat_dim = 64 * 2 + 8 * 6 + 6 * 4
    position_embedding = tf.reshape(kai.nn.new_embedding("position_embedding", dim = feat_dim, expand=SEQ_SIZE, slots=[105]), [-1, SEQ_SIZE, feat_dim])
    ...
    # 下面这些输入的维度加和
    feat_dim = 64 * 2 + 8 * 6 + 6 * 4
    position_embedding = config.new_embedding("position_embedding", dim = feat_dim, slots=[105])
...
if args.mode == 'train':
    position_embedding = tf.layers.dropout(position_embedding, 0.20, training=(args.mode == 'train'))
    attention_input = tf.concat([comment_id_embedding, comment_info_embedding, comment_content_embedding], axis=-1) + position_embedding # [-1, seq, dim]
    user_input = tf.tile(tf.expand_dims(tf.concat([user_embedding, photo_embedding], axis=-1), 1), [1, SEQ_SIZE, 1])   # [-1, seq, dim]
else:
    position_embedding = tf.layers.dropout(position_embedding, 0.20, training=(args.mode == 'train'))
    attention_input = tf.expand_dims(tf.concat([comment_id_embedding, comment_info_embedding, comment_content_embedding], axis=-1) + position_embedding, 0)  # [1, seq, dim]
...
if args.mode == 'train':
    ...
    debug_tensor = {
        ...
        "expand_xtr": tf.reshape(tf.slice(expand_xtr, [0, 0], [32, -1]), [-1, 16]),
        "expand_label": tf.reshape(tf.slice(expand_label, [0, 0], [32, -1]), [-1, 16]),
        "sample_weight": tf.reshape(tf.slice(sample_weight, [0, 0], [32, -1]), [-1, 16]),
        "attention_input_embedding": tf.squeeze(tf.slice(attention_input, [0, 0, 0], [1, -1, 4])),
        "expand_positive_xtr": tf.reshape(tf.slice(tf.boolean_mask(expand_xtr, tf.reduce_sum(expand_label, axis=-1) > 0), [0, 0], [32, -1]), [-1, 16]),
        "expand_positive_label": tf.reshape(tf.slice(tf.boolean_mask(expand_label, tf.reduce_sum(expand_label, axis=-1) > 0), [0, 0], [32, -1]), [-1, 16]),
    }
