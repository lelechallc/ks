...
if args.mode == 'train':
    ...
    # refine expand label
    expand_more_label = tf.where(expand_label > 1, ones, zeros)
    expand_and_like_label = tf.where((expand_label > 0) & (like_label > 0), ones, zeros)
    expand_and_reply_lable = tf.where((expand_label > 0) & (reply_label > 0), ones, zeros)
    expand_label = tf.where((expand_more_label > 0) | (expand_and_like_label > 0) | (expand_and_reply_lable > 0), ones, zeros)
    ...
    eval_targets = [
        ...
        ('expand_more_predict', expand_xtr, expand_more_label, ones, "auc"),
        ('expand_and_like_label', expand_xtr, expand_and_like_label, ones, "auc"),
        ('expand_and_reply_lable', expand_xtr, expand_and_reply_lable, ones, "auc"),
    ]
