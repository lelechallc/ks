...
def main_model(inputs, extra_inputs=None, tower_names=['expand', 'like', 'reply'], tower_units=[256, 128, 64, 1], 
            ...
            projection_units_for_embedding=[]):
    with tf.variable_scope('main_model', reuse=tf.AUTO_REUSE):
        if projection_units_for_embedding:
            for i, unit in enumerate(projection_units_for_embedding):
                inputs = tf.layers.dense(inputs, unit, activation=tf.nn.leaky_relu)
