"""
个性化 model-v3 再再再简化版：v3.6：两层dcn
"""
import os
import argparse
...
if args.mode == 'train':
    ...
    context_emb = kai.nn.new_embedding("context_emb", dim=64, slots=[110, 111, 113])     # mod, page_type
    ...
    c_tag_emb = kai.nn.new_embedding("c_tag_emb", dim=8, slots=[250, 254, 255])
    c_cnt_emb = kai.nn.new_embedding("c_cnt_emb", dim=12, slots=[271, 272, 273, 274, 275, 276, 277, 286, 287, 288, 289, 290, 291])
    c_mmu_score_emb = kai.nn.new_embedding("c_mmu_score_emb", dim=8, slots=[279])
    ...
    ## common attr
    user_profile_emb = config.new_embedding("user_profile_emb", dim=4, slots=[101, 102], **compress_kwargs)  # gender, age
    personalized_id_emb = config.new_embedding("personalized_id_emb", dim=64, slots=[103, 104, 105, 106], **compress_kwargs)  # userid, photo_author_id, photo_id, device_id

    context_emb = config.new_embedding("context_emb", dim=64, slots=[110, 111, 113], **compress_kwargs)     # mod, page_type
    video_profile_emb = config.new_embedding("video_profile_emb", dim=4, slots=[112], **compress_kwargs)    # is_political
    ...
    c_tag_emb = config.new_embedding("c_tag_emb", dim=8, slots=[250, 254, 255])
    c_cnt_emb = config.new_embedding("c_cnt_emb", dim=12, slots=[271, 272, 273, 274, 275, 276, 277, 286, 287, 288, 289, 290, 291])
    c_mmu_score_emb = config.new_embedding("c_mmu_score_emb", dim=8, slots=[279])
...
expand_xtr, like_xtr, reply_xtr, copy_xtr, share_xtr, audience_xtr, continuous_expand_xtr = main_model(
    ...
    tower_units=[128, 64, 1], explicit_cross_layer_num=2, explicit_cross_projection_dim=128,
    explicit_cross_final_proj_dim=512, implicit_cross_layer_units=[], implicit_cross_enable_resnet=False,
    projection_units_for_extra_input=[], explicit_cross_enable_resnet=False
)
...
if args.mode == 'train':
    ...
    # request_time = kai.nn.get_dense_fea("time_ms", dim=1, dtype=tf.int64)
    ...
    #     'expand_predict': (expand_xtr, expand_label),
    #     'like_predict': (like_xtr, like_label),
    #     'reply_predict': (reply_xtr, reply_label),
    #     'copy_predict': (copy_xtr, copy_label),
    #     'share_predict': (share_xtr, share_label),
    #     'audience_predict': (audience_xtr, audience_label),
    #     'continuous_expand_predict': (continuous_expand_xtr, continuous_expand_label),

    # }
    # kai.add_run_hook(DumpTensorHook(table_name='data_for_pcoc_auc', dump_tensors_dict={}), "debug_tensor_hook")
