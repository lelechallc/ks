""" 在v2基础上，把点赞、回复一二级拆开，增加hate、subat、单评论停留时长信号。
    模型结构采用单层PLE，并且每个tower的输入都不包含曝光位置特征！
"""
import os
import argparse
...
def main_model(inputs, tower_names=['expand', 'like', 'reply'], tower_units=[64, 1], tower_last_act={}, 
    ...
    with tf.variable_scope('main_model', reuse=tf.AUTO_REUSE):   
        ...
        for i, tower_name in enumerate(tower_names):
            ...
            if training and tower_name in bias_tower_inputs:
...
expand_xtr, like_first_xtr, like_second_xtr, reply_first_xtr, reply_second_xtr, copy_xtr, share_xtr, audience_xtr, \
    continuous_expand_xtr, hate_xtr, sub_at_xtr, duration_predict= main_model(
    input_wo_pos, task_names, tower_units=[64, 1], tower_last_act={'duration_predict': 'relu'},
    num_shared_experts=3, expert_units=[256, 128], dropout=0.3, training=args.mode=='train', 
    bias_tower_inputs={'duration_predict': input_did_pos}, bias_tower_units=[128, 64, 1], bias_tower_last_act={'duration_predict': 'relu'}
)
