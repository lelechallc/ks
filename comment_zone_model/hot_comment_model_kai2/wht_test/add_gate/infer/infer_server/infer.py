...
# 模型预估目标
PXTRS = ["expand_xtr", "like_xtr", "reply_xtr", "copy_xtr", "share_xtr", "audience_xtr"]
# 最终krp部署时会覆盖这个kess_name
infer_kess_name = "grpc_commentZoneActionInfer_wht"
# 请求带过来的attrs
item_attrs_from_req = [
  "like_cnt", "reply_cnt", "realshow_cnt", "author_id", "minute_diff", "dislike_cnt", "quality_v2_score"
]
common_attrs_from_req = ["photo_id", "photo_author_id"]
model_btq_prefix = "hot_comment_xtr_add_copy_share"
embed_server_shards = 1
# krp 上 embedding_server的kess_name
embed_kess_name = "grpc_commentZoneActionEmb_wht"
...
# Infer服务最终返回的值
service.return_item_attrs(["expand_xtr", "like_xtr", "reply_xtr", "copy_xtr", "share_xtr", "audience_xtr"])
service.add_leaf_flows(leaf_flows = [comment_action_predict_flow], request_type="comment_new_label")
