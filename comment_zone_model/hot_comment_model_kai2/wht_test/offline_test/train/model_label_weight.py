""" 在v2基础上，把点赞、回复一二级拆开，增加hate、subat信号，并且用互动数对loss加权。 使用KAI-IO数据流！
"""
import os
import argparse
...
like_first_xtr = simple_dense_network("like_first_xtr", field_input, [256, 128, 64, 1], 0.0, act=tf.nn.leaky_relu, last_act=tf.nn.sigmoid)
like_second_xtr = simple_dense_network("like_second_xtr", field_input, [256, 128, 64, 1], 0.0, act=tf.nn.leaky_relu, last_act=tf.nn.sigmoid)
reply_first_xtr = simple_dense_network("reply_task_first_xtr", field_input, [256, 128, 64, 1], 0.0, act=tf.nn.leaky_relu, last_act=tf.nn.sigmoid)
reply_second_xtr = simple_dense_network("reply_task_second_xtr", field_input, [256, 128, 64, 1], 0.0, act=tf.nn.leaky_relu, last_act=tf.nn.sigmoid)
...
if args.mode == 'train':
    ...
    # expand_weight = tf.where(expandAction_first > 0, expandAction_first, ones)
    # continuous_expand_weight = tf.where(expandAction_first > 1, expandAction_first - 1, ones)
    ...
    like_second_label = tf.where(likeAction_second > 0, ones, zeros)
    # like_weight = tf.where((likeAction_first - cancelLikeAction_first > 0) | (likeAction_second > 0), likeAction_first - cancelLikeAction_first + likeAction_second, ones)
    # like_first_weight = tf.where(likeAction_first - cancelLikeAction_first > 0, likeAction_first - cancelLikeAction_first, ones)
    # like_second_weight = tf.where(likeAction_second > 0, likeAction_second, ones)
    ...
    # hate_weight = tf.where(hateAction_first - cancelHateAction_first > 0, hateAction_first - cancelHateAction_first, ones)
    ...
    # reply_weight = tf.where((replyTask_first > 0) | (replyTask_second > 0), replyTask_first + replyTask_second, ones)
    # reply_first_weight = tf.where(replyTask_first > 0, replyTask_first, ones)
    # reply_second_weight = tf.where(replyTask_second > 0, replyTask_second, ones)
    ...
    # copy_weight = tf.where((copy_first_label > 0) | (copy_second_label > 0), copy_first_label + copy_second_label, ones)
    ...
    # share_weight = tf.where((share_first_label > 0) | (share_second_label > 0), share_first_label + share_second_label, ones)
    ...
    # audience_weight = tf.where((audience_first_label > 0) | (audience_second_label > 0), audience_first_label + audience_second_label, ones)
    ...
    # sub_at_weight = tf.where((subAtAction_first > 0) | (subAtAction_second > 0), subAtAction_first + subAtAction_second, ones)
    ...
    targets = [
        ...
        # ('like_predict', like_xtr, like_label, like_weight, "auc"),
        ...
        # ('reply_predict', reply_xtr, reply_label, reply_weight, "auc"),
