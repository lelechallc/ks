...
if args.mode == 'train':
    ...
    context_emb = kai.nn.new_embedding("context_emb", dim=64, slots=[110, 111])     # mod, page_type
    ...
    context_emb = config.new_embedding("context_emb", dim=64, slots=[110, 111])     # mod, page_type
...
field_input = tf.concat([user_profile_emb, personalized_id_emb, context_emb, video_profile_emb,
...
if args.mode == 'train':
    ...
    # expand_label = tf.cast(kai.nn.get_dense_fea("expandAction", dim=1, dtype=tf.float32), tf.float32)
    # like_label = tf.cast(kai.nn.get_dense_fea("likeAction", dim=1, dtype=tf.float32), tf.float32)
    # reply_label = tf.cast(kai.nn.get_dense_fea("replyAction", dim=1, dtype=tf.float32), tf.float32)
    ...
    expandAction_first = tf.cast(kai.nn.get_dense_fea("expandAction_first", dim=1, dtype=tf.float32), tf.float32)
    expand_label = tf.where(expandAction_first > 0, ones, zeros)
    continuous_expand_label = tf.where(expandAction_first > 1, ones, zeros)
    
    like_first_label = tf.cast(kai.nn.get_dense_fea("likeAction_first", dim=1, dtype=tf.float32), tf.float32)
    like_second_label = tf.cast(kai.nn.get_dense_fea("likeAction_second", dim=1, dtype=tf.float32), tf.float32)
    like_label = tf.where((like_first_label > 0) | (like_second_label > 0), ones, zeros)

    reply_first_label = tf.cast(kai.nn.get_dense_fea("replyAction_first", dim=1, dtype=tf.float32), tf.float32)
    reply_second_label = tf.cast(kai.nn.get_dense_fea("replyAction_second", dim=1, dtype=tf.float32), tf.float32)
    reply_label = tf.where((reply_first_label > 0) | (reply_second_label > 0), ones, zeros)

    copy_first_label = tf.cast(kai.nn.get_dense_fea("copyAction_first", dim=1, dtype=tf.float32), tf.float32)
    copy_second_label = tf.cast(kai.nn.get_dense_fea("copyAction_second", dim=1, dtype=tf.float32), tf.float32)
    copy_label = tf.where((copy_first_label > 0) | (copy_second_label > 0), ones, zeros)

    share_first_label = tf.cast(kai.nn.get_dense_fea("shareAction_first", dim=1, dtype=tf.float32), tf.float32)
    share_second_label = tf.cast(kai.nn.get_dense_fea("shareAction_second", dim=1, dtype=tf.float32), tf.float32)
    share_label = tf.where((share_first_label > 0) | (share_second_label > 0), ones, zeros)

    audience_first_label = tf.cast(kai.nn.get_dense_fea("audienceaction_first", dim=1, dtype=tf.float32), tf.float32)
    audience_second_label = tf.cast(kai.nn.get_dense_fea("audienceaction_second", dim=1, dtype=tf.float32), tf.float32)
    ...
    recall_type = tf.cast(kai.nn.get_dense_fea("recall_type", dim=1, dtype=tf.float32), tf.float32)
    # ones = tf.ones_like(expand_label, dtype=tf.float32)
    # zeros = tf.zeros_like(expand_label, dtype=tf.float32)

    # realshow_cnt = tf.cast(kai.nn.get_dense_fea("realshow_cnt", dim=1, dtype=tf.float32), tf.float32)
    # vv_80 = tf.where(realshow_cnt > 80, ones, zeros)
    # vv_200 = tf.where(realshow_cnt > 200, ones, zeros)

    comment_genre = tf.cast(kai.nn.get_dense_fea("comment_genre", dim=1, dtype=tf.float32), tf.float32)
