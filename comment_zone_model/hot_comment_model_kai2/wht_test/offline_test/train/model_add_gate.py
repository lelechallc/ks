...
if args.mode == 'train':
    ...
    gate_inputs = [user_profile_emb, personalized_id_emb, context_emb, video_profile_emb]   # dim=396
    gate_inputs_for_emb = [user_profile_emb, context_emb, video_profile_emb]    # dim=140
...
def gate_unit(name, inputs, units):
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        x = inputs
        for i in range(len(units)):
            if i == len(units)-1:
                ...
                act = 'relu'
            x = tf.layers.dense(x, units[i], activation=act)
        return x * 2
    

def tower_module(name, inputs, units, gate_inputs=None):
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        x = inputs
        for i in range(len(units)):
            if i == len(units)-1:
            ...
            if gate_inputs is not None and i < len(units)-1:
                gate_output = gate_unit(f'{name}_gate_{i}', gate_inputs, [x.shape[-1], x.shape[-1]])
                x = x * gate_output
        return x
...
def main_model(inputs, extra_inputs=None, tower_names=['expand', 'like', 'reply'], tower_units=[256, 128, 64, 1], 
            ...
            explicit_cross_enable_resnet=False, gate_inputs=None, gate_inputs_for_emb=None):
    with tf.variable_scope('main_model', reuse=tf.AUTO_REUSE):
        if gate_inputs_for_emb is not None:
            gate_inputs1 = tf.concat([gate_inputs_for_emb], -1)
            gate_output1 = gate_unit(f'embedding_gate1', gate_inputs1, [128, inputs.shape[-1]])
            inputs = inputs * gate_output1
        ...
        if gate_inputs_for_emb is not None:
            gate_inputs2 = tf.concat([gate_inputs_for_emb, tf.stop_gradient(bottom_outputs)], -1)
            gate_output2 = gate_unit(f'embedding_gate2', gate_inputs2, [128, bottom_outputs.shape[-1]])
            bottom_outputs = bottom_outputs * gate_output2

        if gate_inputs is not None:
            gate_inputs = tf.concat([gate_inputs, tf.stop_gradient(bottom_outputs)], -1)
        
        # towers
        outputs=[]
        tower_inputs = bottom_outputs
        for i, tower_name in enumerate(tower_names):
            output = tower_module(tower_name, tower_inputs, tower_units, gate_inputs)
...
gate_inputs = tf.concat(gate_inputs, -1)
gate_inputs_for_emb = tf.concat(gate_inputs_for_emb, -1)
expand_xtr, like_xtr, reply_xtr, copy_xtr, share_xtr, audience_xtr, continuous_expand_xtr = main_model(
    ...
    tower_units=[256, 128, 64, 1], explicit_cross_layer_num=3, explicit_cross_projection_dim=128,
    explicit_cross_final_proj_dim=256, implicit_cross_layer_units=[], implicit_cross_enable_resnet=True,
    projection_units_for_extra_input=[512, 256], explicit_cross_enable_resnet=True, gate_inputs=gate_inputs,
    gate_inputs_for_emb=gate_inputs_for_emb
)

# with tf.variable_scope("model", reuse=tf.AUTO_REUSE):
#     model = Model(input_dim=inputs.shape[-1], tower_names=['expand', 'like', 'reply', 'copy', 'share', 'audience', 'continuous_expand'],
#                 tower_units=[256, 128, 64, 1], explicit_cross_layer_num=3, explicit_cross_projection_dim=256,
#                 explicit_cross_final_proj_dim=256, implicit_cross_layer_units=[512, 256, 256], 
#                 projection_dim_for_extra_input=256)
# pxtrs = model.forward(inputs, extra_inputs=None)
