...
def implicit_cross_layer(name, inputs, units):
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        output = inputs
        for i, unit in enumerate(units):
            output = tf.layers.dense(output, unit, activation=tf.nn.leaky_relu,
...
def main_model(inputs, extra_inputs=None, tower_names=['expand', 'like', 'reply'], tower_units=[256, 128, 64, 1], 
            ...
            implicit_cross_layer_units=[], projection_units_for_extra_input=[]):
    with tf.variable_scope('main_model', reuse=tf.AUTO_REUSE):
        bottom_outputs = []
        if implicit_cross_layer_units:
            implicit_cross_output = implicit_cross_layer('implicit_cross_layer', inputs, implicit_cross_layer_units)
...
# forward
inputs = tf.concat([user_profile_emb, personalized_id_emb, context_emb, video_profile_emb,
...
expand_xtr, like_xtr, reply_xtr, copy_xtr, share_xtr, audience_xtr, continuous_expand_xtr = main_model(
    inputs, extra_inputs=inputs, tower_names=['expand', 'like', 'reply', 'copy', 'share', 'audience', 'continuous_expand'],
    tower_units=[256, 128, 64, 1], explicit_cross_layer_num=3, explicit_cross_projection_dim=256,
    explicit_cross_final_proj_dim=None, implicit_cross_layer_units=[], projection_units_for_extra_input=[]
)
