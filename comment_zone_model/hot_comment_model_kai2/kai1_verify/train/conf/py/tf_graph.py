...
user_embedding = config.new_embedding("user_embedding", dim=4, slots=[101, 102])
...
# interact_comment_id_embedding = tf.reduce_sum(tf.reshape(config.new_embedding("interact_comment_id_embedding", dim=64, expand=10, slots=[105], **compress_kwargs), [-1, 10, 64]), axis=1)
# interact_comment_tag_embedding = tf.reduce_sum(tf.reshape(config.new_embedding("interact_comment_tag_embedding", dim=8, expand=10, slots=[106], **compress_kwargs), [-1, 10, 8]), axis=1)
# write_comment_id_embedding = tf.reduce_sum(tf.reshape(config.new_embedding("write_comment_id_embedding", dim=64, expand=10, slots=[107], **compress_kwargs), [-1, 10, 64]), axis=1)
# write_comment_tag_embedding = tf.reduce_sum(tf.reshape(config.new_embedding("write_comment_tag_embedding", dim=8, expand=10, slots=[108], **compress_kwargs), [-1, 10, 8]), axis=1)
# auhtor_write_comment_id_embedding = tf.reduce_sum(tf.reshape(config.new_embedding("author_write_comment_id_embedding", dim=64, expand=10, slots=[220]), [-1, 10, 64]), axis=1)
# author_write_comment_tag_embedding = tf.reduce_sum(tf.reshape(config.new_embedding("author_write_comment_tag_embedding", dim=8, expand=10, slots=[221]), [-1, 10, 8]), axis=1)
...
field_input = tf.concat([user_embedding, comment_id_embedding, comment_info_embedding,
                         position_embedding
                        #  , comment_label_embedding, comment_tag_embedding,
                        #  interact_comment_id_embedding, interact_comment_tag_embedding,
                        #  write_comment_id_embedding, write_comment_tag_embedding,
                        #  auhtor_write_comment_id_embedding, author_write_comment_tag_embedding
                         ], -1)
...
if args.mode == 'train':
    ...
    expand_label = config.get_label("expandAction_v")
    like_label = config.get_label("likeAction_v")
    reply_label = config.get_label("replyAction_v")
