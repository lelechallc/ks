...
def convert_pytorch_checkpoint_to_tf(model: BertForMaskedLM, ckpt_dir: str, model_name: str):
    ...
    with tf.Session() as session:
        for var_name in state_dict:
            if var_name in ['cls.predictions.decoder.weight', 'cls.predictions.decoder.bias']:
                continue
...
def convert(pytorch_bin_path: str, pytorch_bin_model: str, tf_ckpt_path: str, tf_ckpt_model: str):
    ...
    model = BertForMaskedLM.from_pretrained(
        ...
        state_dict=torch.load(os.path.join(pytorch_bin_path, pytorch_bin_model), map_location='cpu'),config = './bert_config.json'
    )
