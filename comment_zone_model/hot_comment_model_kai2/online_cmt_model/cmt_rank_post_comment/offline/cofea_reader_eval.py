...
read_data = DataReaderFlow(name="read_data") \
    .fetch_message(
        ...
        hdfs_path="viewfs:///home/reco_algorithm/dw/reco_algorithm.db/comment_model_data_wht/data_set=20250120_21_22_22h_add",
...
gen_feature = DataReaderFlow(name="gen_feature") \
    ...
    .enrich_attr_by_lua(
        import_item_attr=["comment_stay_time", "comment"],
        function_for_item="get_label",
        export_item_attr=["comment_after_read"],
        lua_script=f"""
            function get_label()
                local comment_after_read = 0
                if comment_stay_time > 3000  and comment == 1 then
                    comment_after_read = 1
                end
                return comment_after_read
            end
        """
    ) \
    .enrich_attr_by_lua(
...
send_mio = DataReaderFlow(name="send_mio") \
    ...
    .send_to_mio_learner(
        attrs=labels + ['recall_type', 'comment_genre', 'hate_label'] + other_labels + ['comment', 'comment_after_read'],
