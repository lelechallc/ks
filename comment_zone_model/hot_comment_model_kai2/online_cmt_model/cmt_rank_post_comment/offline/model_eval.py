...
# sub_staytime_predict = simple_dense_network("sub_staytime_predict", input_wo_pos, [256, 128, 64, 1], 0.0, act=tf.nn.leaky_relu, last_act=tf.nn.relu)
# depth_xtr = simple_dense_network("depth_xtr", input_wo_pos, [256, 128, 64, 1], 0.0, act=tf.nn.leaky_relu, last_act=tf.nn.sigmoid)
# page_time_xtr = simple_dense_network("page_time_predict", input_wo_pos, [256, 128, 64, 1], 0.0, act=tf.nn.leaky_relu, last_act=tf.nn.sigmoid)
...
comment_xtr = simple_dense_network("comment_after_read_xtr", input_wo_pos, [256, 128, 64, 1], 0.0, act=tf.nn.leaky_relu, last_act=tf.nn.sigmoid)
...
if args.mode == 'train' or args.mode == 'eval':
    ...
    comment_after_read = kai.nn.get_dense_fea("comment_after_read", dim=1, dtype=tf.float32)
    comment_after_read_label =  tf.where(comment_after_read > 0.0, ones, zeros)
    ...
    targets = [
        ...
        ('comment_after_read_predict', comment_xtr, comment_after_read_label, ones* depthMask, "auc"),
    ]
    ...
    targets = [
        ...
        ('comment_after_read_predict', comment_xtr, "auc"),
    ]
