...
# 模型预估目标
PXTRS = ["expand_xtr", "like_xtr", "reply_xtr", "copy_xtr", "share_xtr", "audience_xtr", "continuous_expand_xtr", 'duration_predict','page_time_pxtr', "hate_xtr", "report_xtr"]
# 最终krp部署时会覆盖这个kess_name
infer_kess_name = "grpc_HotCommentPageInferGpu"
...
model_btq_prefix = "cmt_rank_page"


embed_server_shards = 2
# krp 上 embedding_server的kess_name
# embed_kess_name = "grpc_HotCommentXtrAddDurationEmb"
colossusdb_embd_service_name="hot_comment_rank_colossusdb"
colossusdb_embd_table_name="cmt-rank-page-emb-table2"
...
class PredictServerFlow(LeafFlow, KuibaApiMixin, MioApiMixin, OfflineApiMixin, GsuApiMixin, UniPredictApiMixin, KgnnApiMixin, SeRecoApiMixin):
  ...
  def predict_with_mio_model(self, **kwargs):
    ...
    class PredictSubFlow(LeafFlow, KuibaApiMixin, MioApiMixin, OfflineApiMixin, GsuApiMixin, UniPredictApiMixin, KgnnApiMixin, SeRecoApiMixin):
       pass
    predict_subflow = PredictSubFlow(name="predict_sub_flow") \
      ...
      .uni_predict_fused(
        ...
        model_loader_config=dict(							 						## 模型加载相关的配置
            ...
            implicit_batch=implicit_batch,		#gpu部署 false						# 是否为 implicit batch，implicit batch 的情况下不支持 XLA 和 Step2 中的 compress_group
            receive_dnn_model_as_macro_block=receive_dnn_model_as_macro_block,
            enable_xla = False, # gpu部署 false
        ),
        batching_config = dict(
          batch_timeout_micros=4500,  # gpu部署 0->4500 
          max_batch_size=max(batch_sizes),
          max_enqueued_batches=1,   #
          # batch_task_type="BatchTensorflowTask",  # 使用tf-cpu 部署 需要显示指定 BatchTensorflowTask， gpu部署注释掉
          enable_pinned_memory=True,  # gpu部署 
        ),
        ...
        inputs=fix_inputs + [dict(
                ...
                # common=common,
                dim=dim,
              ) for attr_name, tensor_name, dim in extra_inputs], #for attr_name, tensor_name, common, dim in extra_inputs
    ...
    return self \
      .arrange_by_sub_flow(sub_flow=predict_subflow, expected_partition_size=80)
...
# Infer服务最终返回的值
service.return_item_attrs(["expand_xtr", "like_xtr", "reply_xtr", "copy_xtr", "share_xtr", "audience_xtr", "continuous_expand_xtr", 'duration_predict', 'page_time_pxtr', "hate_xtr", "report_xtr"])
service.add_leaf_flows(leaf_flows = [comment_action_predict_flow], request_type="hot_comment_default")
service.build(output_file=os.path.join(current_folder, "infer_gpu_config.json"))
