...
fuse_input = gate_input * action_input + (1 - gate_input) * content_input

expand_xtr = simple_dense_network("expand_xtr", fuse_input, [64, 1], 0.0, act=tf.nn.leaky_relu, last_act=tf.nn.sigmoid)
like_xtr = simple_dense_network("like_xtr", fuse_input, [64, 1], 0.0, act=tf.nn.leaky_relu, last_act=tf.nn.sigmoid)
reply_xtr = simple_dense_network("reply_xtr", fuse_input, [64, 1], 0.0, act=tf.nn.leaky_relu, last_act=tf.nn.sigmoid)
copy_xtr = simple_dense_network("copy_xtr", fuse_input, [64, 1], 0.0, act=tf.nn.leaky_relu, last_act=tf.nn.sigmoid)
share_xtr = simple_dense_network("share_xtr", fuse_input, [64, 1], 0.0, act=tf.nn.leaky_relu, last_act=tf.nn.sigmoid)
audience_xtr = simple_dense_network("audience_xtr", fuse_input, [64, 1], 0.0, act=tf.nn.leaky_relu, last_act=tf.nn.sigmoid)
continuous_expand_xtr = simple_dense_network("continuous_expand_xtr", fuse_input, [64, 1], 0.0, act=tf.nn.leaky_relu, last_act=tf.nn.sigmoid)
...
if args.mode == 'train' or args.mode == 'eval':
    ...
    margin = 0.6
    # 映射到同一向量空间
    cid_trans = simple_dense_network("cid_trans", comment_id_embedding, [128], 0, act=None, last_act= None)
    bert_trans =simple_dense_network("bert_trans", bert_output_emb, [128], 0, act=None, last_act= None)
    # l2 norm
    cid_emb_norm = tf.nn.l2_normalize(cid_trans, dim = -1)
    bert_emb_norm = tf.nn.l2_normalize(bert_trans, dim = -1)
    # 计算余弦相似度
    neg_ratio  = 0.1 # 负采样率
    inner_prod = tf.matmul(cid_emb_norm, bert_emb_norm, transpose_b=True)  # (B, B)
    pos_loss = 1 - inner_prod # (B, B)
    neg_loss = tf.maximum(tf.zeros_like(inner_prod), margin - (1 - inner_prod)) # (B, B)
    random_tensor = tf.random_uniform(tf.shape(inner_prod), minval=0, maxval=1)
    random_dig_part = tf.diag_part(random_tensor) # 对角线向量
    random_diag = tf.matrix_diag(random_dig_part) #  对角线原值，其余位置变0
    random_diff = random_tensor - random_diag # 对角线变0，其余位置是random_tensor原值
    mask_neg = tf.greater(random_diff, 1- neg_ratio) # 对 > (1-neg_ratio)的值采样，因为范围均在0-1之间，对角线全0，相当于对非对角线位置neg_ratio概率采样
    mask_pos = tf.greater(tf.diag(tf.ones(tf.shape(inner_prod)[0])),0) # (B, B)
    contrastive_loss = tf.reduce_mean(tf.where(mask_pos, pos_loss, tf.zeros_like(pos_loss))) 
    contrastive_loss += tf.reduce_mean(tf.where(mask_neg, neg_loss, tf.zeros_like(pos_loss))) 
