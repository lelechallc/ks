...
import bert.modeling_last_layer as modeling
...
if args.mode == 'train' or args.mode == 'eval':
    ...
    bert_first5_layers_emb = kai.nn.get_dense_fea("bert_first5_layers_emb", dim=27*256, dtype=tf.float32)
    ...
    bert_first5_layers_emb = config.get_extra_param(name="bert_first5_layers_emb", size=27*256)
...
layer_6_inputs = tf.reshape(bert_first5_layers_emb, [-1,27,256])
bert_config = modeling.BertConfig.from_json_file(bert_config_file)
model = modeling.BertModel(
    config=bert_config,
    is_training=True,    
    input_ids=token_input_ids, 
    layer_6_inputs = layer_6_inputs, 
