...
effe_read_xtr = simple_dense_network("effe_read_xtr", input_wo_pos, [256, 128, 64, 1], 0.3, act=tf.nn.leaky_relu, last_act=tf.nn.sigmoid)
readtime_xtr = simple_dense_network("readtime_xtr", input_wo_pos, [256, 128, 64, 1], 0.3, act=tf.nn.leaky_relu, last_act=tf.nn.sigmoid)
...
if args.mode == 'train' or args.mode == 'eval':
    ...
    efferead = kai.nn.get_dense_fea("efferead", dim=1, dtype=tf.float32)
    readtime_flag = kai.nn.get_dense_fea("readtime_flag", dim=1, dtype=tf.float32)
    readtime = kai.nn.get_dense_fea("readtime", dim=1, dtype=tf.float32)
    ...
    targets = [
        ...
        ('efferead_predict', effe_read_xtr, efferead, ones * readtime_flag, "auc"),
        ('read_time_predict', readtime_xtr, ones, readtime * readtime_flag, "auc"),
        ('read_time_predict_neg', readtime_xtr, zeros, ones, "auc"),
    ]
    ...
    trans_readtime = readtime_xtr / (1 - readtime_xtr)
    ...
    eval_targets = [
        ...
        ('report_predict', report_xtr, report_label, ones, "auc"),
        ('efferead_predict', effe_read_xtr, efferead, ones * readtime_flag, "auc"),
        ('time_predict', trans_readtime, readtime, ones * readtime_flag, 'linear_regression'),
    ...
    targets = [
        ...
        ('efferead_xtr', effe_read_xtr),
        ("staytime_gauss_predict", readtime_xtr / (1 - readtime_xtr)),
    ]
