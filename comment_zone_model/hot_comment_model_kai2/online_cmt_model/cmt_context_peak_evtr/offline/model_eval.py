...
peak_xtr = simple_dense_network("peak_xtr", input_wo_pos, [256, 128, 64, 1], 0.0, act=tf.nn.leaky_relu, last_act=tf.nn.sigmoid)
...
if args.mode == 'train' or args.mode == 'eval':
    ...
    peak_la = kai.nn.get_dense_fea("peak_effective_label", dim=1, dtype=tf.float32)
    peak_label = tf.where(peak_la > 0.0, ones, zeros)
    ...
    targets = [
        ...
        ('peak_predict', peak_xtr, peak_label, ones, "auc"),
    ]
