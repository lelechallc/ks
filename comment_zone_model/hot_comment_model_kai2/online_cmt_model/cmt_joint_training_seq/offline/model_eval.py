...
if args.mode == 'train' or args.mode == 'eval':
    ...
    interact_comment_ids_emb = kai.new_embedding("interact_comment_id_embedding", dim=64, expand=10, slots=[112])
    interact_comment_tags_emb = kai.new_embedding("interact_comment_tag_embedding", dim=16, expand=10, slots=[113])
    ...
    comment_tag_embedding = kai.new_embedding("comment_tag_embedding", dim=16, slots=[253])
    ...
    # -----new
    interact_comment_ids_emb = config.new_embedding("interact_comment_id_embedding", dim=64, expand=10, slots=[112], **compress_kwargs)
    interact_comment_tags_emb = config.new_embedding("interact_comment_tag_embedding", dim=16, expand=10, slots=[113], **compress_kwargs)
    # ------
    ...
    # new
    comment_tag_embedding = config.new_embedding("comment_tag_embedding", dim=16, slots=[253])
...
def simple_dense_network(name, inputs, units, dropout=0, act=tf.nn.tanh, last_act=tf.nn.sigmoid, stop_gradient=False):
    ...
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        ...
        return output
def dot_product_attention_layer(name, photo_list, key_dim, query, query_dim, attn_size, out_size):
  ...
  with tf.variable_scope(name,reuse=tf.AUTO_REUSE):
    ...
    attention_res = tf.layers.dense(attention_res, out_size, activation=tf.nn.tanh,
                                  kernel_initializer=tf.glorot_uniform_initializer(), name= "att_out_layer")
...
cid_query = tf.concat([comment_id_embedding, comment_tag_embedding], axis = -1)
interact_seq = tf.concat([tf.reshape(interact_comment_ids_emb, [-1, 10, 64]), tf.reshape(interact_comment_tags_emb, [-1, 10, 16])],axis = -1)
attn_size = 64
out_size = 64
taeget_att = dot_product_attention_layer("target_attention", interact_seq, interact_seq.shape.as_list()[-1], cid_query, cid_query.shape.as_list()[-1], attn_size, out_size)
...
input_wo_pos = tf.concat([input_wo_pos, bert_output_emb, taeget_att], -1) # 最简单的联合方式：concat
