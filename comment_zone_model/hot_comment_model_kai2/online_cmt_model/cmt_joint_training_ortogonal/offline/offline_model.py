...
if args.mode == 'train' or args.mode == 'eval':
    ...
    tf.summary.scalar("total_loss0", tf.reduce_mean(loss))
    loss_duration = tf.losses.huber_loss(duration_label, duration_predict, weights=1.0, delta=3.0)
    print("loss_duration_shape", loss_duration.shape)
    loss = loss + loss_duration
    print("loss_shape1", loss.shape)
    ...
    inner_prod = tf.reduce_sum(tf.multiply(cid_emb_norm, bert_emb_norm), 1, True)
    ortogonal_loss  = tf.pow(inner_prod,2)
    print("ortogonal_loss_shape", ortogonal_loss.shape)
    tf.summary.scalar("ortogonal_loss", tf.reduce_mean(ortogonal_loss))
    tf.summary.scalar("total_loss", tf.reduce_mean(loss))
    loss += 1000 * ortogonal_loss
