...
fm_input  =tf.concat([user_embedding, uid_embedding, did_embedding, bert_output_emb, comment_length_embedding])
fm_fileds = tf.shape(fm_input)[1] / 8
fm_in = tf.reshape(fm_input, shape=[-1, fm_fileds, 8])
fm_out = batch_group_fm_quadratic(fm_in)
...
input_wo_pos = tf.concat([input_wo_pos, bert_output_emb, fm_out], -1) # 最简单的联合方式：concat
