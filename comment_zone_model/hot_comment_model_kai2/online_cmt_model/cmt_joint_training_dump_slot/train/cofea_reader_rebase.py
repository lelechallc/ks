...
gen_feature = DataReaderFlow(name="gen_feature") \
    ...
    .enrich_attr_by_lua(  # 权重：是否有评论区停留时长信号
        import_item_attr=["comment_stay_time"],
        export_item_attr=["comment_stay_time", "readtime_flag"],
        function_for_item="calculate",
        lua_script="""
            function calculate()
                local comment_stay_time = comment_stay_time or 0
                return comment_stay_time, comment_stay_time > 0
            end
        """
    ) \
    .enrich_attr_by_lua(  # 计算单次收益
        import_item_attr=[
            "likeAction_first", "likeAction_second", "replyAction_first", "replyAction_second", 
            "audienceAction_first", "audienceAction_second", "copyAction_first", "copyAction_second", 
            "shareAction_first", "shareAction_second", "expandAction_first"
        ],
        function_for_item="get_reward",
        export_item_attr=["reward"],
        lua_script="""
            function get_reward()
                local like = 1.5 * likeAction_first + 1.5 * likeAction_second
                local reply = 1.5 * replyAction_first + 1.5 * replyAction_second
                local audie = audienceAction_first + audienceAction_second
                local copy = copyAction_first + copyAction_second
                local share = shareAction_first + shareAction_second
                local expand = 1.5 * math.min(expandAction_first, 3)
                return 1.0 + like + reply + audie + copy + share + expand
            end
        """
    ) \
    .pack_item_attr(
        ...
        mappings = [
            {
                "from_item_attr": "likeAction_first",
                "to_common_attr": "like1_list",
            }, {
                "from_item_attr": "likeAction_second",
                "to_common_attr": "like2_list",
            }, {
                "from_item_attr": "replyAction_first",
                "to_common_attr": "reply1_list",
            }, {
                "from_item_attr": "replyAction_second",
                "to_common_attr": "reply2_list",
            }, {
                "from_item_attr": "audienceAction_first",
                "to_common_attr": "audi1_list",
            }, {
                "from_item_attr": "audienceAction_second",
                "to_common_attr": "audi2_list",
            }, {
                "from_item_attr": "copyAction_first",
                "to_common_attr": "copy1_list",
            }, {
                "from_item_attr": "copyAction_second",
                "to_common_attr": "copy2_list",
            }, {
                "from_item_attr": "shareAction_first",
                "to_common_attr": "share1_list",
            }, {
                "from_item_attr": "shareAction_second",
                "to_common_attr": "share2_list",
            }, {
                "from_item_attr": "expandAction_first",
                "to_common_attr": "expand_list",
            }
        ]
    ) \
    .enrich_attr_by_lua(  # 计算总收益
        import_common_attr=[
            "like1_list", "like2_list", "reply1_list", "reply2_list", 
            "audi1_list", "audi2_list", "copy1_list", "copy2_list",
            "share1_list", "share2_list", "expand_list"
        ],
        export_common_attr= ["total_reward"],
        function_for_common="get_total_reward",
        lua_script="""
            function get_total_reward()
                local total_reward = 0.0
                for i=1, #like1_list do
                    local like = 1.5 * like1_list[i] + 1.5 * like2_list[i]
                    local reply = 1.5 * reply1_list[i] + 1.5 * reply2_list[i]
                    local audie = audi1_list[i] + audi2_list[i]
                    local copy = copy1_list[i] + copy2_list[i]
                    local share = share1_list[i] + share2_list[i]
                    local expand = 1.5 * math.min(expand_list[i], 3)
                    local reward = 1.0 + like + reply + audie + copy + share + expand
                    total_reward = total_reward + reward
                end
                total_reward = math.max(total_reward, #like1_list)
                return total_reward
            end
        """
    ) \
    .enrich_attr_by_lua(  # 标签：mock 阅读时长
        import_common_attr=["total_reward"],
        import_item_attr=["reward", "comment_stay_time"],
        export_item_attr=["readtime"],
        function_for_item="get_time",
        lua_script="""
            function get_time()
                local readtime = (comment_stay_time / 1000.0) * reward / total_reward
                return math.min(readtime, 30)
            end
        """
    ) \
    .enrich_attr_by_lua(  # 标签：有效阅读
        import_item_attr=["readtime"],
        function_for_item="get_eff_read",
        export_item_attr=["efferead"],
        lua_script="""
            function get_eff_read()
                local efferead = readtime >= 1.5
                return efferead
            end
        """
    ) \
    ...
    .log_debug_info(
            item_attrs = ['bert_id', 'comment_content','token_input_ids', 'token_input_mask', 'token_output', 'token_sep_ids'],
...
send_mio = DataReaderFlow(name="send_mio") \
    ...
    .send_to_mio_learner(
        attrs=labels + ['recall_type', 'comment_genre', 'hate_label'] + ["token_input_ids", "token_input_mask", "token_sep_ids"]+ other_labels + ['readtime', 'efferead', 'readtime_flag'],
