...
if args.mode == 'train':
    ...
    eval_targets = [
        ...
        # ('expand_hot_predict', expand_xtr, expand_label, tf.where(recall_type < 1.1, ones, zeros), "auc"),
        # ('like_hot_predict', like_xtr, like_label, tf.where(recall_type < 1.1, ones, zeros), "auc"),
        # ('reply_hot_predict', reply_xtr, reply_label, tf.where(recall_type < 1.1, ones, zeros), "auc"),


        # ('expand_climb_predict', expand_xtr, expand_label, tf.where((recall_type > 1.1) & (recall_type < 4.1), ones, zeros), "auc"),
        # ('like_climb_predict', like_xtr, like_label, tf.where((recall_type > 1.1) & (recall_type < 4.1), ones, zeros), "auc"),
        # ('reply_climb_predict', reply_xtr, reply_label, tf.where((recall_type > 1.1) & (recall_type < 4.1), ones, zeros), "auc"),
    ]
