""" 在v2基础上，增加单评论停留时长信号。并且每个tower的输入都不包含曝光位置特征！和 model_add_duration_with_ple 比仅结构没有用ple！
"""
import os
import argparse
...
# sub_staytime_predict = simple_dense_network("sub_staytime_predict", input_wo_pos, [256, 128, 64, 1], 0.3, act=tf.nn.leaky_relu, last_act=tf.nn.relu)
# depth_xtr = simple_dense_network("depth_xtr", input_wo_pos, [256, 128, 64, 1], 0.3, act=tf.nn.leaky_relu, last_act=tf.nn.sigmoid)
page_time_xtr = simple_dense_network("page_time_predict", input_wo_pos, [256, 128, 64, 1], 0.3, act=tf.nn.leaky_relu, last_act=tf.nn.sigmoid)
...
if args.mode == 'train' or args.mode == 'eval':
    ...
    # sub_staytime_predict = sub_staytime_predict + duration_pos_bias_predict
    ...
    # sub_staytime_label = kai.nn.get_dense_fea("stayDurationMs_second", dim=1, dtype=tf.float32)
    # sub_staytime_label = tf.clip_by_value(sub_staytime_label / 1000, 0, 60)

    # depthAction = kai.nn.get_dense_fea("depthAction", dim=1, dtype=tf.float32)
    depthMask = kai.nn.get_dense_fea("depthMask", dim=1, dtype=tf.float32)

    page_time_label =  kai.nn.get_dense_fea("comment_stay_time", dim=1, dtype=tf.float32)
    page_time_label = page_time_label / 1000
    ...
    targets = [
        ...
        # ('depth_predict', depth_xtr, ones, depthAction * depthMask, "auc"),
        # ('depth_predict_neg', depth_xtr, zeros, ones * depthMask, "auc"),
        ('page_time_predict', page_time_xtr, ones, page_time_label * depthMask, "auc"),
        ('page_time_predict_neg', page_time_xtr, zeros, ones * depthMask, "auc"),
    ...
    # loss_sub_staytime = tf.losses.huber_loss(sub_staytime_label, sub_staytime_predict, weights=1.0, delta=3.0)
    # loss = loss + loss_sub_staytime
    ...
    eval_targets = [
        ...
        # ('sub_staytime_predict', sub_staytime_predict, sub_staytime_label, ones, 'linear_regression'),
        # ('depth_predict', depth_xtr, depthAction, ones * depthMask, "auc"),
        ('page_time_predict', page_time_xtr/(1-page_time_xtr), page_time_label, ones * depthMask, "linear_regression"),
        ('hate_predict', hate_xtr, hate_label, ones, "auc"),
        ('report_predict', report_xtr, report_label, ones, "auc"),
        ...
        ('page_time_climb', page_time_xtr/(1-page_time_xtr), page_time_label, tf.where(tf.less_equal(recall_type, 4.1), tf.where(tf.less_equal(1.1, recall_type), ones, zeros), zeros), 'linear_regression'),
        ...
        ('report_climb', report_xtr, report_label, tf.where(tf.less_equal(recall_type, 4.1), tf.where(tf.less_equal(1.1, recall_type), ones, zeros), zeros), 'auc'),

        ('expand_hot', expand_xtr, expand_label, tf.where(tf.equal(recall_type, 1), ones, zeros), "auc"),
        ('like_hot', like_xtr, like_label, tf.where(tf.equal(recall_type, 1), ones, zeros), "auc"),
        ('reply_hot', reply_xtr, reply_label, tf.where(tf.equal(recall_type, 1), ones, zeros), "auc"),
        ('copy_hot', copy_xtr, copy_label, tf.where(tf.equal(recall_type, 1), ones, zeros), "auc"),
        ('share_hot', share_xtr, share_label, tf.where(tf.equal(recall_type, 1), ones, zeros), "auc"),
        ('audience_hot', audience_xtr, audience_label, tf.where(tf.equal(recall_type, 1), ones, zeros), "auc"),
        ('continuous_expand_hot', continuous_expand_xtr, continuous_expand_label, tf.where(tf.equal(recall_type, 1), ones, zeros), "auc"),
        ('duration_hot', duration_predict, duration_label, tf.where(tf.equal(recall_type, 1), ones, zeros), 'linear_regression'),
        ('page_time_hot', page_time_xtr/(1-page_time_xtr), page_time_label, tf.where(tf.equal(recall_type, 1), ones, zeros), 'linear_regression'),
        ('hate_hot', hate_xtr, hate_label, tf.where(tf.equal(recall_type, 1), ones, zeros), "auc"),
        ('report_hot', report_xtr, report_label, tf.where(tf.equal(recall_type, 1), ones, zeros), 'auc'),
    ]
    ...
    targets = [
        ...
        # ("sub_staytime_predict", sub_staytime_predict),
        # ("depth_xtr", depth_xtr),
        ("page_time_pxtr", page_time_xtr/(1-page_time_xtr)),
        ('hate_xtr', hate_xtr),
