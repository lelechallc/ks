...
read_data = DataReaderFlow(name="read_data") \
    .fetch_message(
        ...
        hdfs_path="viewfs:///home/reco_algorithm/dw/reco_algorithm.db/comment_model_data_wht/data_set=20250113_13_1h",
...
other_labels =["depthAction", "depthMask", "depthInteract"]
other_label = DataReaderFlow(name="other_label") \
    .enrich_attr_by_lua(
        import_item_attr=["expandAction_first", "expandAction_second","replyAction_first", "replyAction_second",
            "likeAction_first", "likeAction_second", "audienceAction_first", "audienceAction_second",
            "reportAction_first", "reportAction_second", "copyAction_first", "copyAction_second", 
            "shareAction_first", "shareAction_second",
            'hateAction_first', 'hateAction_second',
            "cancelHateAction_first", "cancelHateAction_second", 'cancelLikeAction_first', 'cancelLikeAction_second'],
        function_for_item="get_point_interact_num",
        export_item_attr=["point_interact_num"],
        lua_script=f"""
            function get_point_interact_num()
                local interact_num  = 0
                interact_num = expandAction_first + expandAction_second + replyAction_first + replyAction_second + likeAction_first
                    + likeAction_second  + audienceAction_first + audienceAction_second + reportAction_first + reportAction_second
                    + copyAction_first + copyAction_second + shareAction_first + shareAction_second
                return interact_num
            end
        """
    ) \
    ...
    .pack_item_attr(
        ...
        mappings=[
            ...
            {
                "aggregator": "sum",
                "from_item_attr": "point_interact_num",
                "to_common_attr": "context_interact_num",
            },
        ]
    ) \
    .enrich_attr_by_lua(
        import_common_attr=["firstCmtShowCnt", "context_interact_num"],
        export_item_attr=["depthAction", "depthMask", "depthInteract"],   # 曝光个数, 互动次数
        function_for_item="cal_depth",
        lua_script="""
            function cal_depth(seq, item_key, reason, score)
                local depthAction = 0
                local mask = 0
                local depthInteract  = 0
                
                if seq == 0 then
                    depthAction = math.max(firstCmtShowCnt - 4, 0)
                    depthInteract = context_interact_num
                    mask = 1
                end
                
                return depthAction, mask, depthInteract
            end
        """
    ) \
    .perflog_attr_value(
        check_point="other.label",
        item_attrs=["showAction", "depthAction", "depthMask", "seqAction", "depthInteract", "point_interact_num"],
        common_attrs=["max_showAction", "firstCmtShowCnt", "secondCmtShowCnt", "context_interact_num"],
    )
