...
if 'kafka' in data_sources:
  fetch_message_configs.update(dict(
    ...
    group_id='slide_multi_task_interact_pt_after_cmt',
...
.enrich_attr_by_lua(
  ...
  .enrich_attr_by_lua(
      ...
      lua_script = """
          function calculate()
              user_comment_cluster_level = tonumber(user_comment_cluster_level) or 0
...
send_to_mio_learner = DataReaderFlow(name = "send_to_mio_learner") \
  .send_to_mio_learner(
    attrs=labels+["interact_weight", "interact_label","playing_time","playing_time_s","comment_stay_time_s","user_comment_cluster_level",
...
runner.IGNORE_UNUSED_ATTR=["comment_watch_num_label_div_list_str", "emp_click_comment_rate_list", "emp_comment_consume_depth_list", "emp_comment_stay_duration_list", "user_ids"]
