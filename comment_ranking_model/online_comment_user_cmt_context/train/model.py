...
def uid_cluster_gate(name, inputs, units, dropout=0, act=tf.nn.sigmoid, last_act=tf.nn.sigmoid, stop_gradient=False):
    if stop_gradient:
        output = tf.stop_gradient(inputs)
    else:
        last_output = inputs
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        final_outputs = []
        if dropout > 0:
            last_output = tf.layers.dropout(last_output, dropout, training=(args.mode == 'train'))
        for i, unit in enumerate(units):
            # output = tf.layers.Dense(unit, act, name='dense_{}_{}'.format(name, i))(output)
            if i == len(units) - 1:
                act = last_act
            output = tf.layers.dense(last_output, unit, activation=tf.nn.relu,
                                  kernel_initializer=tf.glorot_uniform_initializer())
            output = tf.layers.dense(output, unit, activation=last_act,
                                  kernel_initializer=tf.glorot_uniform_initializer())
            final_outputs.append(2 * output)
            last_output = output
        return final_outputs

def simple_dense_network_wiz_gate(name, inputs, gate_weights, units, dropout=0, act=tf.nn.tanh, last_act=tf.nn.sigmoid, stop_gradient=False):
    if stop_gradient:
        output = tf.stop_gradient(inputs)
    else:
        output = inputs
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        ...
        for i, unit in enumerate(units):
            ...
            if i == len(units) - 1:
                act = last_act
            unit_gate_weights = gate_weights[i]
            output = tf.multiply(output, unit_gate_weights)
            output = tf.layers.dense(output, unit, activation=act,
                                  kernel_initializer=tf.glorot_uniform_initializer())
        return output
    
def simple_dense_network_bottom_wiz_gate(name, inputs, gate_weights, units, dropout=0, act=tf.nn.tanh, last_act=tf.nn.sigmoid, stop_gradient=False):
    if stop_gradient:
        output = tf.stop_gradient(inputs)
    else:
        output = inputs
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        ...
        for i, unit in enumerate(units):
            ...
            if i == 0:
                ...
                output = tf.multiply(output, gate_weights[i-1])
                output = tf.layers.dense(output, unit, activation=act,
                                    kernel_initializer=tf.glorot_uniform_initializer())
        return output
# photo features
...
user_action_comment_ids_seq = config.new_embedding('user_action_comment_list_id', dim=32, slots=[922], **compress_kwargs)
user_action_comment_ids_seq = tf.reshape(user_action_comment_ids_seq, [-1, 1, 32])
photo_top_comment_ids_seq = config.new_embedding('photo_top_comment_list_id', dim=32, expand=50, slots=[923], **compress_kwargs)
photo_top_comment_ids_seq = tf.reshape(photo_top_comment_ids_seq, [-1, 50, 32])
...
cid_seq_output = multi_head_attention("cid_seq_attention", user_action_comment_ids_seq, photo_top_comment_ids_seq, num_heads=4, num_units=32)    #[batchsize, 1, 32]

seq_output = tf.concat([like_seq_output, follow_seq_output, forward_seq_output, comment_seq_output, collect_seq_output, profile_enter_seq_output, cid_seq_output], axis=2) #[batchsize, 1, 32*6] infer: [1, batchsize, 32*6]
seq_output = tf.reshape(seq_output, [-1, 32*7])
...
user_app_cluster_level = config.get_label("user_app_cluster_level")
gate_inputs_4_cmt = tf.concat([user_app_cluster_level], axis=1)
gate_inputs_4_others = tf.concat([user_app_cluster_level, uid_emb, pid_emb, aid_emb], axis=1)
...
with tf.name_scope('model'):
    ...
    # follow_cluster_gate = uid_cluster_gate("follow_cluster_gate", gate_inputs_4_others, [256,128,64])
    # follow_logit =  simple_dense_network_bottom_wiz_gate("follow_logit", feature_input, follow_cluster_gate, [256, 128, 64, 1], act=tf.nn.leaky_relu, last_act=tf.nn.leaky_relu) 
    # forward_inside_cluster_gate = uid_cluster_gate("forward_inside_cluster_gate", gate_inputs_4_others, [256,128,64])
    # forward_inside_logit =  simple_dense_network_bottom_wiz_gate("forward_inside_logit", feature_input, forward_inside_cluster_gate, [256, 128, 64, 1], act=tf.nn.leaky_relu, last_act=tf.nn.leaky_relu) 
    # interact_cluster_gate = uid_cluster_gate("interact_cluster_gate", gate_inputs_4_others, [256,128,64])
    # interact_logit =  simple_dense_network_bottom_wiz_gate("interact_logit", feature_input, interact_cluster_gate, [256, 128, 64, 1], act=tf.nn.leaky_relu, last_act=tf.nn.leaky_relu) 
    # click_comment_cluster_gate = uid_cluster_gate("click_comment_cluster_gate", gate_inputs_4_cmt, [256,128,64])
    # click_comment_logit =  simple_dense_network_bottom_wiz_gate("click_comment_logit", feature_input, click_comment_cluster_gate, [256, 128, 64, 1], act=tf.nn.leaky_relu, last_act=tf.nn.leaky_relu) 
    # comment_time_score_cluster_gate = uid_cluster_gate("comment_time_score_cluster_gate", gate_inputs_4_cmt, [256,128,64])
    # comment_time_score_logit =  simple_dense_network_bottom_wiz_gate("comment_time_score_logit", feature_input, comment_time_score_cluster_gate, [256, 128, 64, 1], act=tf.nn.leaky_relu, last_act=tf.nn.leaky_relu) 
    comment_top_net_cluster_gate = uid_cluster_gate("comment_top_net_cluster_gate", gate_inputs_4_cmt, [256])
    comment_top_net =  simple_dense_network_bottom_wiz_gate("comment_top_net", feature_input, comment_top_net_cluster_gate, [256, 128], act=tf.nn.leaky_relu, last_act=tf.nn.leaky_relu) 
    ...
    comment_unfold_score_logit_cluster_gate = uid_cluster_gate("comment_unfold_score_logit_cluster_gate", gate_inputs_4_cmt, [128, 64])
    comment_unfold_score_logit =  simple_dense_network_wiz_gate("comment_unfold_score_logit", comment_top_net, comment_unfold_score_logit_cluster_gate, [64, 1], act=tf.nn.leaky_relu, last_act=tf.nn.leaky_relu) 
    comment_like_score_logit_cluster_gate = uid_cluster_gate("comment_like_score_logit_cluster_gate", gate_inputs_4_cmt, [128, 64])
    comment_like_score_logit =  simple_dense_network_wiz_gate("comment_like_score_logit", comment_top_net, comment_like_score_logit_cluster_gate, [64, 1], act=tf.nn.leaky_relu, last_act=tf.nn.leaky_relu) 
    comment_content_copyward_score_logit_cluster_gate = uid_cluster_gate("comment_content_copyward_score_logit_cluster_gate", gate_inputs_4_cmt, [128, 64])
    comment_content_copyward_score_logit =  simple_dense_network_wiz_gate("comment_content_copyward_score_logit", comment_top_net, comment_content_copyward_score_logit_cluster_gate, [64, 1], act=tf.nn.leaky_relu, last_act=tf.nn.leaky_relu) 
    comment_effective_read_score_logit_cluster_gate = uid_cluster_gate("comment_effective_read_score_logit_cluster_gate", gate_inputs_4_cmt, [128, 64])
    comment_effective_read_score_logit =  simple_dense_network_wiz_gate("comment_effective_read_score_logit", comment_top_net, comment_effective_read_score_logit_cluster_gate, [64, 1], act=tf.nn.leaky_relu, last_act=tf.nn.leaky_relu) 
    comment_slide_down_score_logit_cluster_gate = uid_cluster_gate("comment_slide_down_score_logit_cluster_gate", gate_inputs_4_cmt, [128, 64])
    comment_slide_down_score_logit =  simple_dense_network_wiz_gate("comment_slide_down_score_logit", comment_top_net, comment_slide_down_score_logit_cluster_gate, [64, 1], act=tf.nn.leaky_relu, last_act=tf.nn.leaky_relu) 
    # uplift_comment_consume_depth_score_logit_cluster_gate = uid_cluster_gate("uplift_comment_consume_depth_score_logit_cluster_gate", gate_inputs_4_cmt, [128, 64])
    # uplift_comment_consume_depth_score_logit =  simple_dense_network_wiz_gate("uplift_comment_consume_depth_score_logit", comment_top_net, uplift_comment_consume_depth_score_logit_cluster_gate, [64, 1], act=tf.nn.leaky_relu, last_act=tf.nn.leaky_relu) 
    # uplift_comment_stay_duration_score_logit_cluster_gate = uid_cluster_gate("uplift_comment_stay_duration_score_logit_cluster_gate", gate_inputs_4_cmt, [128, 64])
    # uplift_comment_stay_duration_score_logit =  simple_dense_network_wiz_gate("uplift_comment_stay_duration_score_logit", comment_top_net, uplift_comment_stay_duration_score_logit_cluster_gate, [64, 1], act=tf.nn.leaky_relu, last_act=tf.nn.leaky_relu) 
    # effective_read_comment_fresh_logit_cluster_gate = uid_cluster_gate("effective_read_comment_fresh_logit_cluster_gate", gate_inputs_4_cmt, [128, 64])
    # effective_read_comment_fresh_logit =  simple_dense_network_wiz_gate("effective_read_comment_fresh_logit", comment_top_net,  effective_read_comment_fresh_logit_cluster_gate, [64, 1], act=tf.nn.leaky_relu, last_act=tf.nn.leaky_relu) 

    # eft_click_cmt_logit_cluster_gate = uid_cluster_gate("eft_click_cmt_logit_cluster_gate", gate_inputs_4_cmt, [128, 64])
    # eft_click_cmt_logit =  simple_dense_network_wiz_gate("eft_click_cmt_logit", comment_top_net,  eft_click_cmt_logit_cluster_gate, [64, 1], act=tf.nn.leaky_relu, last_act=tf.nn.leaky_relu) 
    # eft_write_cmt_logit_cluster_gate = uid_cluster_gate("eft_write_cmt_logit_cluster_gate", gate_inputs_4_cmt, [128, 64])
    # eft_write_cmt_logit =  simple_dense_network_wiz_gate("eft_write_cmt_logit", comment_top_net,  eft_write_cmt_logit_cluster_gate, [64, 1], act=tf.nn.leaky_relu, last_act=tf.nn.leaky_relu) 
    ...
    # comment_genre_layer_cluster_gate = uid_cluster_gate("comment_genre_layer_cluster_gate", gate_inputs_4_cmt, [256])
    # comment_genre_layer =  simple_dense_network_bottom_wiz_gate("comment_genre_layer", feature_input, comment_genre_layer_cluster_gate, [256, 128], act=tf.nn.leaky_relu, last_act=tf.nn.leaky_relu) 
    # sub_comment_score_cluster_gate = uid_cluster_gate("sub_comment_score_cluster_gate", gate_inputs_4_cmt, [128, 64])
    # sub_comment_score_logit =  simple_dense_network_wiz_gate("sub_comment_score_logit", comment_genre_layer,  sub_comment_score_cluster_gate, [64, 1], act=tf.nn.leaky_relu, last_act=tf.nn.leaky_relu) 
    # emoji_comment_score_cluster_gate = uid_cluster_gate("emoji_comment_score_cluster_gate", gate_inputs_4_cmt, [128, 64])
    # emoji_comment_score_logit =  simple_dense_network_wiz_gate("emoji_comment_score_logit", comment_genre_layer,  emoji_comment_score_cluster_gate, [64, 1], act=tf.nn.leaky_relu, last_act=tf.nn.leaky_relu) 
    # gif_comment_score_cluster_gate = uid_cluster_gate("gif_comment_score_cluster_gate", gate_inputs_4_cmt, [128, 64])
    # gif_comment_score_logit =  simple_dense_network_wiz_gate("gif_comment_score_logit", comment_genre_layer,  gif_comment_score_cluster_gate, [64, 1], act=tf.nn.leaky_relu, last_act=tf.nn.leaky_relu) 
    # at_comment_score_cluster_gate = uid_cluster_gate("at_comment_score_cluster_gate", gate_inputs_4_cmt, [128, 64])
    # at_comment_score_logit =  simple_dense_network_wiz_gate("at_comment_score_logit", comment_genre_layer,  at_comment_score_cluster_gate, [64, 1], act=tf.nn.leaky_relu, last_act=tf.nn.leaky_relu) 
    # image_comment_score_cluster_gate = uid_cluster_gate("image_comment_score_cluster_gate", gate_inputs_4_cmt, [128, 64])
    # image_comment_score_logit =  simple_dense_network_wiz_gate("image_comment_score_logit", comment_genre_layer,  image_comment_score_cluster_gate, [64, 1], act=tf.nn.leaky_relu, last_act=tf.nn.leaky_relu) 
    # text_comment_score_cluster_gate = uid_cluster_gate("text_comment_score_cluster_gate", gate_inputs_4_cmt, [128, 64])
    # text_comment_score_logit =  simple_dense_network_wiz_gate("text_comment_score_logit", comment_genre_layer,  text_comment_score_cluster_gate, [64, 1], act=tf.nn.leaky_relu, last_act=tf.nn.leaky_relu) 
    # video_comment_score_cluster_gate = uid_cluster_gate("video_comment_score_cluster_gate", gate_inputs_4_cmt, [128, 64])
    # video_comment_score_logit =  simple_dense_network_wiz_gate("video_comment_score_logit", comment_genre_layer,  video_comment_score_cluster_gate, [64, 1], act=tf.nn.leaky_relu, last_act=tf.nn.leaky_relu) 
    ...
    original_logits = [follow_logit, forward_inside_logit, interact_logit, click_comment_logit, comment_time_score_logit, 
                    ...
                    effective_read_comment_fresh_logit,
    ...
    text_comment_score, video_comment_score, uplift_comment_consume_depth_score, comment_slide_down_score,uplift_comment_stay_duration_score, \
...
if args.mode == 'train':
    ...
    total_loss = tf.losses.log_loss(labels, preds, weights, reduction="weighted_sum") + tpm_train_losss_op + progressive_effective_click_comment_loss + long_view_counter_factual_regularization_loss
    ...
    debug_tensor = {
        ...
        "low_user_comment_cluster_level":tf.slice(low_user_comment_cluster_level, [0, 0], [20, -1]),
        "mid_user_comment_cluster_level":tf.slice(mid_user_comment_cluster_level, [0, 0], [20, -1]),
        "high_user_comment_cluster_level":tf.slice(high_user_comment_cluster_level, [0, 0], [20, -1]),
        "user_comment_cluster_level" : tf.slice(user_comment_cluster_level, [0, 0], [20, -1]),
    }
    ...
    targets = [
        ...
        ('playtime_after_click_comment_score', long_view_counter_factual_score_cmt), ##暂时放弃playtime_after_click_comment_score塔的训练
