...
read_joint_reco_log = DataReaderFlow(name="read_joint_reco_log") \
  ...
  .gsu_common_colossusv2_enricher(kconf="colossus.kconf_client.video_item",
        ...
        item_fields=dict(photo_id="photo_id_list",
                          ...
                          label="label_list")) \
  .enrich_attr_by_lua(
    import_common_attr=["timestamp_list", "time_gap_window", "time_ms_common", "label_list", "play_time_list", "photo_id_list", "profile_stay_time_list", "duration_list"],
  ...
  .copy_attr(
    attrs=[{
      "from_item": "photo_id_emb",
      "to_item": "photo_id_emb_cp"
    }]
  ) \
  .pack_item_attr(
    item_source={
    ...
    mappings = [{
      "from_item_attr":"photo_id_emb_cp",
...
post_process = DataReaderFlow(name="post_process") \
  ...
  .enrich_attr_by_lua(
    import_common_attr=["photo_emb_list", "label_list", "play_time_list", "profile_stay_time_list", "duration_list", "emb_similarity_threshold"],
    import_item_attr=["photo_id_emb"],
    export_item_attr=["like_value", "follow_value", "forward_value", "comment_value", "profile_enter_value", "play_time", "profile_stay_time", "long_play_value", "effective_play_value", "short_play_value", "negative_action_value"],
    function_for_item="agg",
    lua_script="""
      ...
      function agg()
        ...
        local profile_enter_count = 0
        local long_play_count = 0
        local effective_play_count = 0
        local short_play_count = 0
        local neg_action_count = 0
        local play_time = 0
        local profile_stay_time = 0
        for i = 1, #photo_emb_list, 64 do
          local ab = 0
          local aa = 0
          local bb = 0
          for j = 1, 64 do
            ab = ab + (photo_emb_list[i + j] or 0) * (photo_id_emb[j] or 0)
            aa = aa + (photo_emb_list[i + j] or 0) * (photo_emb_list[i + j] or 0)
            bb = bb + (photo_id_emb[j] or 0) * (photo_id_emb[j] or 0)
          end
          local sim = ab * ab / aa / bb
          if sim > emb_similarity_threshold then
            local idx = math.floor((i+63) / 64)
            like_count = like_count + ((label_list[idx] & (1 << 0)) > 0 and 1 or 0)
            follow_count = follow_count + ((label_list[idx] & (1 << 1)) > 0 and 1 or 0)
            forward_count = forward_count + ((label_list[idx] & (1 << 2)) > 0 and 1 or 0)
            comment_count = comment_count + ((label_list[idx] & (1 << 4)) > 0 and 1 or 0)
            profile_enter_count = profile_enter_count + ((label_list[idx] & (1 << 6)) > 0 and 1 or 0)
            long_play_count = long_play_count + (isLongPlay(duration_list[idx], play_time_list[idx]) and 1 or 0)
            effective_play_count = effective_play_count + (isEffectivePlay(duration_list[idx], play_time_list[idx]) and 1 or 0)
            short_play_count = short_play_count + (isShortPlay(duration_list[idx], play_time_list[idx]) and 1 or 0)
            neg_action_count = neg_action_count + ((label_list[idx] & (1 << 13)) > 0 and 1 or 0)
            play_time = play_time + play_time_list[idx]
            profile_stay_time = profile_stay_time + profile_stay_time_list[idx]
          end
        end
        return like_count, follow_count, forward_count, comment_count, profile_enter_count, play_time, profile_stay_time, long_play_count, effective_play_count, short_play_count, neg_action_count
      end
    """
  ) \
  .log_debug_info(
