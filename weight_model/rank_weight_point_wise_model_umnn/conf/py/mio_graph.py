...
def weight_predtict_umnn_model(user_seq, photo_category, pxtrs):
  user_intent = intent_predictor(user_seq, 16)
  user_intent = simple_dense_network("intent_emb", user_intent, [16], act= tf.nn.sigmoid)
  context_pxtr = ranking_ensemble_umnn_model(pxtrs, user_intent, photo_category)
  sorted_by_pxtr1_pxtrs, orl_pxtrs, integral_loss = umnn_integralNN(pxtrs,context_pxtr)
  pred_ensemble_score = umnn_ensemble(sorted_by_pxtr1_pxtrs)
  orl_ensemble_score = tf.reduce_prod(orl_pxtrs, axis = 1, keepdims=True)
  return pred_ensemble_score, orl_ensemble_score, integral_loss

def ranking_ensemble_umnn_model(pxtrs, user_intent, photo_category):
  ...
  pxtrs_dim = pxtrs.get_shape().as_list()[1]  
  context_pxtr = unmm_network("context_pxtr", concat_reslut_wiz_pxtr, [256, 128, 64, pxtrs_dim])
...
def weight_predtict_umnn_model_with_pair(user_seq, photo_category_idx0, photo_category_idx1, pxtrs_idx0, pxtrs_idx1):
  user_intent = intent_predictor(user_seq, 16)
  user_intent = simple_dense_network("intent_emb", user_intent, [16], act= tf.nn.sigmoid)
  context_pxtr_idx0, context_pxtr_idx1 = ranking_ensemble_umnn_model_with_pair(pxtrs_idx0, pxtrs_idx1, user_intent, photo_category_idx0, photo_category_idx1)
  sorted_by_pxtr1_pxtrs_idx0, sorted_by_pxtr1_pxtrs_idx1 = umnn_integralNN(pxtrs_idx0, pxtrs_idx1,context_pxtr_idx0, context_pxtr_idx1)
  # pred_score_idx0, pred_score_idx1 = relative_quantile_ensemble_with_pair(pxtr_weights_idx0, pxtr_weights_idx1, pxtrs_idx0, pxtrs_idx1)
  return sorted_by_pxtr1_pxtrs_idx0, sorted_by_pxtr1_pxtrs_idx1
...
def ranking_ensemble_model_with_pair(pxtrs_idx0, pxtrs_idx1, user_intent, photo_category_idx0, photo_category_idx1):
  ...
  weights_idx0 = simple_dense_network("projection_idx0", concat_reslut_idx0, [6], act= tf.nn.elu)
  weights_idx1 = simple_dense_network("projection_idx1", concat_reslut_idx1, [6], act= tf.nn.elu)
  return weights_idx0, weights_idx1

def ranking_ensemble_umnn_model_with_pair(pxtrs_idx0, pxtrs_idx1, user_intent, photo_category_idx0, photo_category_idx1):
  pxtrs_idx0_expand = tf.expand_dims(pxtrs_idx0, 1)
  pxtrs_idx1_expand = tf.expand_dims(pxtrs_idx1, 1)
  user_intent_expand = tf.expand_dims(user_intent, 1)
  photo_category_idx0_expand = tf.expand_dims(photo_category_idx0, 1)
  ...
  concat_reslut_wiz_pxtr_idx0 = tf.concat([concat_reslut_idx0, pxtrs_idx0[:,0]], axis = 1)
  concat_reslut_wiz_pxtr_idx1 = tf.concat([concat_reslut_idx1, pxtrs_idx1[:,0]], axis = 1)

  context_pxtr_idx0 = unmm_network("context_pxtr_idx0", concat_reslut_wiz_pxtr_idx0, [256,128,64,len(pxtrs_idx0[:,0])])
  context_pxtr_idx1 = unmm_network("context_pxtr_idx1", concat_reslut_wiz_pxtr_idx1, [256,128,64,len(pxtrs_idx1[:,0])])
  return context_pxtr_idx0, context_pxtr_idx1
...
if args.mode == 'train':
  ...
  print_op = tf.cond(
    ...
    lambda: tf.print(
              ...
              ", pxtr_weights_pltr_idx0", tf.slice(pxtr_weights_idx0, [0, 0], [20, 1]),
              ", pxtr_weights_pwtr_idx0", tf.slice(pxtr_weights_idx0, [0, 1], [20, 1]),
              ", pxtr_weights_pftr_idx0", tf.slice(pxtr_weights_idx0, [0, 2], [20, 1]),
              ", pxtr_weights_pcmtr_idx0", tf.slice(pxtr_weights_idx0, [0, 3], [20, 1]),
              ", pxtr_weights_plvtr_idx0", tf.slice(pxtr_weights_idx0, [0, 4], [20, 1]),
              ", pxtr_weights_pct_idx0", tf.slice(pxtr_weights_idx0, [0, 5], [20, 1]),
  ...
  optimizer = tf.train.GradientDescentOptimizer(0.2, name="opt")
