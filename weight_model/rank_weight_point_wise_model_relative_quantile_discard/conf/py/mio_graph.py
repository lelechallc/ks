...
if args.mode == 'train':
  ...
  targets = [
    #('good_quality', level_reward_pos, one, 10 * tf.square(pred_score - level_reward), 'auc'),
  ...
  eval_targets = [
    ('good_quality', level_reward_pos, one, one, 'auc'),
    # ('interact_label',tf.slice(pred_score, [0, 0], [-1, 1]), interact_label, one, 'auc'),
    # ('click_comment_label',tf.slice(pred_score, [0, 1], [-1, 1]), slide_click_comment_button, one, 'auc'),
    # ('long_view', tf.slice(pred_score, [0, 2], [-1, 1]), long_view, one, 'auc'),
    # ('effective_view', tf.slice(pred_score, [0, 3], [-1, 1]), effective_view, one, 'auc'),
    # ('follow', tf.slice(pred_score, [0, 4], [-1, 1]), follow, one, 'auc'),
  ...
  #loss = tf.reduce_sum(tf.square(pred_score - level_reward)) + log_loss
  loss = log_loss
