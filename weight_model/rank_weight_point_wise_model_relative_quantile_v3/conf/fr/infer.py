...
PXTRS = ["like_weight", "follow_weight", "forward_weight", "comment_weight", "longview_weight", "effective_view_weight"]
# 最终krp部署时会覆盖这个kess_name relative_quantile_weight_
infer_kess_name = "grpc_relativeQuantileWeightInfer"
# 请求带过来的attrs
pxtr_list = ["pltr","pwtr", "pftr", "pcmtr", "plvtr", "pctr","pcltr","pdtr"]
pxtrs_list = [xtr+'s' for xtr in pxtr_list]
pxtr_pair_list = [pxtr+"_idx0" for pxtr in pxtr_list] 
pxtr_pair_list += [pxtr+"_idx1" for pxtr in pxtr_list]
pxtrs_pair_list = [xtr for xtr in pxtr_pair_list]
...
model_btq_prefix = "relative_quantile_weight_model_v2"
embed_server_shards = 2
# krp 上 embedding_server的kess_name
embed_kess_name = "grpc_relativeQuantileWeightModelEmbedV2"
...
class PredictServerFlow(LeafFlow, KuibaApiMixin, MioApiMixin, OfflineApiMixin, GsuApiMixin, UniPredictApiMixin, KgnnApiMixin):
  ...
  def predict_with_mio_model(self, **kwargs):
    ...
    extra_inputs = []
    for xtr, xtrs in zip(pxtr_pair_list, pxtrs_pair_list):
    ...
    predict_subflow = PredictSubFlow(name="predict_sub_flow") \
      ...
      .enrich_attr_by_lua(
        ...
        export_item_attr=pxtrs_pair_list,
        lua_script="""
          function tolist()
            ...
            return {pltr}, {pwtr}, {pftr},  {pcmtr}, {plvtr}, {pctr}, {pltr}, {pwtr}, {pftr},  {pcmtr}, {plvtr}, {pctr}
          end
        """) \
      .uni_predict_fused(
        embedding_fetchers = [
